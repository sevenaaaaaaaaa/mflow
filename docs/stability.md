...
## 八、配额预警（T6，2026-09-17 上线）

| 触发点 | 阈值 | 行为 |
|--------|------|------|
| 用量记账时（`usage_add`） | **≥80%** | 飞书 ⚠ 预警（含用户/指标/用量/百分比 + 处置建议） |
| 同上 | **≥100%** | 飞书 ⛔ 超额告警 |
| 去重 | 每用户 × 每指标 × 每月 × 每档 | `warned` 标记，只发一次，不刷屏 |
| 健康检查 | — | `quota_alerts` 进入健康报告 → 总览健康卡显示；级别升为 warn/bad |
| UI | — | 设置页配额表按 80%/100% 标黄/标红 |

覆盖指标：`items`（条目）· `tokens` · `writes`（真实写入）。
实测：5 条配额下写 4 条 → 80% 预警；写第 5 条 → 100% 告警；tokens 90% → 预警（未被 100% 重复触发）。
